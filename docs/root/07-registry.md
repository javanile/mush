...
The `.packages` file lists all the packages that will be installed or updated through the `mush install` command. The main file is hosted at [Mush Repository](https://github.com/javanile/mush/blob/main/.packages), and everyone can request a pull to this file to add their packages or registries.
